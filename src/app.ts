...
async function start() {
  ...
  server.get(
    `/picture/:size(${sizes.join("|")})/:id.:format(jpg|webp)`,
    imageHandler
  );
  server.get(`/picture/:id.:format(jpg|webp)`, imageHandler);

  server.get(
    `/:lang([a-z]{2})-:country([a-z]{2})/:size(${sizes.join(
      "|"
    )})/:name.:format(jpg|webp)`,
    imageHandler
  );
  server.get(
    `/:lang([a-z]{2})/:size(${sizes.join("|")})/:name.:format(jpg|webp)`,
    imageHandler
  );
  server.get(
    `/:lang([a-z]{2})-:country([a-z]{2})/:name.:format(jpg|webp)`,
    imageHandler
  );
  server.get(
    `/:size(${sizes.join("|")})/:name.:format(jpg|webp)`,
    imageHandler
  );
